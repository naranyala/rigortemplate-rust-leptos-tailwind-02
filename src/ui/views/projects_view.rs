...
pub fn ProjectsView() -> impl IntoView {
    let projects = vec![
        ("Cloud Migration", "In Progress", "indigo"),
        ("API v2 Rewrite", "Completed", "green"),
        ("Mobile App", "Planning", "amber"),
        ("Auth System", "Testing", "blue"),
    ];

    view! {
        <div class="p-6 space-y-6">
            <div class="flex items-center justify-between">
                <h2 class="text-2xl font-bold text-slate-800">"Project Portfolio"</h2>
                ...
                {projects.into_iter().map(|(name, status, color)| {
                    view! {
                        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all group">
                            <div class=format!("w-2 h-12 rounded-full bg-{}", color)></div>
                            <div class="ml-4">
